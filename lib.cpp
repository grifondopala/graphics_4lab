...
void DrawBezierCubic(MyPoint& p0, MyPoint& p1, MyPoint& p2, MyPoint& p3, Mat& image, Vec3b color) {
    drawPolygon({ p0, p1, p2, p3 }, image, RED);
...
int CyrusBeckClipLine(int& x1, int& y1, int& x2, int& y2, double& t1, double& t2, vector<MyPoint>& points, int n) {
    ...
    for (int i = 0; i < n; i++) {
        nx = points[(i + 1) % n].y - points[i].y; ny = points[i].x - points[(i + 1) % n].x; // нормаль к стороне полигона
        ...
        else { if (Classify(points[i].x, points[i].y, points[(i + 1) % n].x, points[(i + 1) % n].y, x1, y1) == LEFT) return 0; } // параллельны
    } //for
...
void drawCyrusBeckLine(MyPoint& p1, MyPoint& p2, vector<MyPoint>& points, Mat& image) {
    ...
    if (CyrusBeckClipLine(x1, y1, x2, y2, t1, t2, points, points.size() - 1)) {
        ...
        if (t1 != 0) {
            drawLine(p1.x, p1.y, x1, y1, image, RED);
        }

        if (t2 != 1) {
            drawLine(x2, y2, p2.x, p2.y, image, RED);
        }

        drawLine(x1, y1, x2, y2, image, BLUE);
    }
    else {
        drawLine(x1, y1, x2, y2, image, RED);
    }
}